...
def step_when_customer_places_order(context):
    ...
    for row in context.table:
        ...
        category = (
            ProductCategory(category_str) if category_str else ProductCategory.DEFAULT
        )
        product = Product(
            name=row["productName"], unit_price=int(row["unitPrice"]), category=category
        )
...
def step_then_order_summary_should_be(context):
    ...
    assert context.order.total_amount == expected_total, (
        f"Expected total to be {expected_total}, but got {context.order.total_amount}"
    )

    if "originalAmount" in context.table[0]:
        expected_original_amount = int(context.table[0]["originalAmount"])
        assert context.order.original_amount == expected_original_amount, (
            f"Expected original amount to be {expected_original_amount}, but got {context.order.original_amount}"
        )
    ...
    if "discount" in context.table[0]:
        ...
        assert context.order.discount == expected_discount, (
            f"Expected discount to be {expected_discount}, but got {context.order.discount}"
        )
...
def step_then_customer_should_receive(context):
    for row in context.table:
        ...
        assert found, (
            f"Expected to find product {product_name} with quantity {quantity}"
        )
...
def step_given_threshold_discount(context):
    if not hasattr(context, "promotions"):
        context.promotions = {}
    context.promotions[PromotionType.THRESHOLD_DISCOUNT.value] = {
...
def step_given_double_11_promotion_active(context):
    if not hasattr(context, "promotions"):
        context.promotions = {}
    context.promotions[PromotionType.DOUBLE_11.value] = True
